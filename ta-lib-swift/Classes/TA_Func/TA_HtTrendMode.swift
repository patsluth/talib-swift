...
public class TA_HtTrendMode
{
                                ...
                                outInteger: inout [Int]) -> TA_ReturnCode
    {
        ...
        let allocationSize = self.allocationSize(startIndex: startIndex,
                                                 endIndex: endIndex)
        outInteger = [Int](repeating: NSNotFound, count: allocationSize)
        ...
        while true {
            ...
            idx = today
            tempReal = 0.0
            for _ in stride(from: 0, to: DCPeriodInt, by: 1) {
                tempReal += inDouble[idx]
                idx -= 1
            }
            if DCPeriodInt > 0 {
                tempReal /= Double(DCPeriodInt)
            }
            let trendline = ((((4.0 * tempReal) + (3.0 * iTrend1)) + (2.0 * iTrend2)) + iTrend3) / 10.0
            iTrend3 = iTrend2
            iTrend2 = iTrend1
            iTrend1 = tempReal
            var trend = 1
            ...
            daysInTrend += 1
            if daysInTrend < Int(0.5 * smoothPeriod) {
                trend = 0
            }
            ...
            if trendline != 0.0 && (fabs(Double((tempReal - trendline) / trendline)) >= 0.015) {
                trend = 1
            }
            if today >= startIndex {
                                ...
                                outInteger: inout [Int]) -> TA_ReturnCode
    {
        ...
        while true {
            ...
            tempReal = 0.0
            for _ in stride(from: 0, to: DCPeriodInt, by: 1) {
                tempReal += inCandle[idx][inCandleInputType]
                idx -= 1
            }
            if DCPeriodInt > 0 {
                tempReal /= Double(DCPeriodInt)
    ...
    {
        return TA_Core.sharedInstance.globals.unstablePeriod[TA_FunctionUnstableId.HtTrendMode.rawValue] + 63
    }
